...
class MerchantHttpTest extends TestCase
{
    ...
    public function test_get_status()
    {
        ...
        $this->assertEquals(round($between->sum('subtotal'), 2), $response['revenue']);
        $this->assertEquals(round($between->sum('commission_owed') - $noAffiliate->commission_owed, 2), $response['commissions_owed']);
    }
}
